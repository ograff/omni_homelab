...
- **Tailscale-exposed services:** use `type: LoadBalancer` + `loadBalancerClass: tailscale` (tailscale operator is installed and watches for this class). Used for internal-only services that shouldn't go through the Cloudflare tunnel. To expose an existing service on tailscale, flip `type`/`loadBalancerClass` on the existing service — don't add a second service alongside it. Nginx ingress backends work fine against a LoadBalancer service, and the older bjw-s common library (used by gabe565 charts like `paperless-ngx`) emits a container port per service-port, so two services sharing a port number produce duplicate `containerPort` entries and ArgoCD fails with `duplicate entries for key [containerPort=...]`. Caveat: that same old common library (bjw-s common ≤1.5.x) silently drops `loadBalancerClass` from `service.*` values — if the chart uses it, add a raw `Service` in the wrapper's `templates/` dir instead of a values override.
